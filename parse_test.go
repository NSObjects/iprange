...
func TestParseRange(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "v4 Range: invalid start",
			input:   "192.0.2.-192.0.2.10",
			wantErr: assert.Error,
		},
		{
			name:    "v4 Range: invalid end",
			input:   "192.0.2.0-192.0.2",
			wantErr: assert.Error,
		},
		{
			name:    "v4 Range: v6 start",
			input:   "2001:db8::0-192.0.2.10",
			wantErr: assert.Error,
		},
		{
			name:    "v4 Range: v6 end",
		...
		{
			name:      "v4 CIDR: ip instead of host address",
			input:     "192.0.2.10/24",
			wantRange: prepareRange("192.0.2.1", "192.0.2.254"),
			wantErr:   assert.NoError,
		},
		{
			name:    "v4 CIDR: missing prefix length",
			input:   "192.0.2.0/",
			wantErr: assert.Error,
		},
		{
			name:    "v4 CIDR: invalid prefix length",
			input:   "192.0.2.0/99",
			wantErr: assert.Error,
		},
		{
		...
		{
			name:    "v4 Mask: missing prefix mask",
			input:   "192.0.2.0/",
			wantErr: assert.Error,
		},
		{
			name:    "v4 Mask: invalid mask",
			input:   "192.0.2.0/mask",
			wantErr: assert.Error,
		},
		{
			name:    "v4 Mask: not canonical form mask",
			input:   "192.0.2.0/255.255.0.254",
			wantErr: assert.Error,
		},
		{
			name:    "v4 Mask: v6 address",
		...
		{
			name:    "v6 Range: invalid start",
			input:   "2001:db8-2001:db8::10",
			wantErr: assert.Error,
		},
		{
			name:    "v6 Range: invalid end",
			input:   "2001:db8::-2001:db8",
			wantErr: assert.Error,
		},
		{
			name:    "v6 Range: v4 start",
			input:   "192.0.2.0-2001:db8::10",
			wantErr: assert.Error,
		},
		{
			name:    "v6 Range: v4 end",
		...
		{
			name:      "v6 CIDR: ip instead of host address",
			input:     "2001:db8::10/64",
			wantRange: prepareRange("2001:db8::1", "2001:db8::ffff:ffff:ffff:fffe"),
			wantErr:   assert.NoError,
		},
		{
			name:    "v6 CIDR: missing prefix length",
			input:   "2001:db8::/",
			wantErr: assert.Error,
		},
		{
			name:    "v6 CIDR: invalid prefix length",
			input:   "2001:db8::/999",
			wantErr: assert.Error,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.wantErr(t, err, fmt.Sprintf("ParseRange(%v)", tt.input)) {
				return
			}
			assert.Equalf(t, tt.wantRange, got, "ParseRange(%v)", tt.input)
		})
	}
}
